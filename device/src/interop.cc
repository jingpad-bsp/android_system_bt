...
static const char* interop_feature_string_(const interop_feature_t feature) {
  switch (feature) {
    ...
    CASE_RETURN_STR(INTEROP_CVSD_ONLY)
    CASE_RETURN_STR(INTEROP_ARVCP_REJ_DEALY)
    CASE_RETURN_STR(INTEROP_SLC_STATE_CHLD_SEND)
    CASE_RETURN_STR(INTEROP_HF_CLIENT_AT_CMD)
    CASE_RETURN_STR(INTEROP_DISABLE_AAC_CODEC_SUPPORT)
#endif
  }
