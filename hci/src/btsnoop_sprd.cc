...
void btsnoop_open(void) {
    ...
    if (btsnoop_fd != INVALID_FD) {
        SNOOPD("btsnoop log file is already open,%s bte_trace_level_switch: 5", value);
    ...
    for (i =0; i < CTRL_CHAN_RETRY_COUNT; i++) {
       ...
       if (btsnoop_fd >= 0) {
               bte_trace_level_switch(1);
               SNOOPE("%s connect successs: %d", LOG_SOCKET, btsnoop_fd);
       } else if ( !strstr(value, USER_DEBUG_VERSION_STR)){
               bte_trace_level_switch(0);
               SNOOPE("%s user connect failed: %d", LOG_SOCKET, btsnoop_fd);
               hci_logging_enabled = 0;
               return;
       } else {
                bte_trace_level_switch(1);
                SNOOPE("%s userdebug connect failed: %d", LOG_SOCKET, btsnoop_fd);
       }
 }
    ...
    if (events & EPOLLIN) {
        ...
        if (ret > 0) {
            if(0 == strncmp(buf, LOG_TURN_ON, sizeof(LOG_TURN_ON))){
                ...
                btsnoop_open();
                touch_btsnoop_fd();
            } else if (0 == strncmp(buf, LOG_TURN_OFF, sizeof(LOG_TURN_OFF))) {
...
static int is_btsnoop_enable(void) {
    ...
    ret = property_get("sys.ylog.svc.hcidump", hcidump_property, "unknow");
...
EXPORT_SYMBOL extern const module_t btsnoop_sprd_module = {
